...
$(document).ready(function(){
   ...
   item_num = utils.getUrlParameter("item");
   preview = new Preview(item_num); // позже должно всместо 13 проставляться автоматически
   user = new User();
...
function setupDom(){
   ...
   $("#catalog_search_input").keypress(function(e){
      if(e.keyCode==13){
         location.href = "/catalog.php?page=1&query="+$("#catalog_search_input").val()+(region.current_region != -1 ? "&region="+region.current_region : "")+(region.current_city != -1 ? "&city="+region.current_city : "")+(region.current_city != -1 ? "&city="+region.current_city : "");
      }
   });

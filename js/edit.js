...
function Newitem(){
   ...
   this.selected_currency = 0; // default is 0 = dollar
   ...
   this.init = function(){
      $('#content-area').load("./dom/item_edit.html", this.oninit);
   ...
   this.oninit = function(){
      $(document).click(function(e){
         ...
         if (!target.is('#metro_list') && 
         !target.is('#metro_input')) $('#metro_list').hide();
      });
      
      newitem.resizeUploadImg = function(i){
      ...
      newitem.upload = $('#item_photo_upload_a').upload({
         ...
         onComplete: function(data){
            ...
            console.log("upload.onComplete; "+data);
   ...
   this.fillForm = function(item){
      $.post(post_url,{
         ...
         for (var i = 0; i < metro.length; i++)
            if (metro[i].metro_code == obj.metro && metro[i].city_code == obj.city){
               region.current_metro = obj.metro;
               region.setSearchMetro(obj.city, obj.metro, metro[i].title, 'metro_input');
            }
   ...
   this.fillMetroList = function(){
      $('#metro_list').html("");
      $('#metro_list').show();
      region.fillMetroList('metro_list', 'metro_input');
   };
   ...
   this.checkNotChecked = function(){
      if ($('input.radGroup2:checked').length == 0){
         newitem.errors++;
         newitem.hlText($('label.radGroup2'));
       
         if(!newitem.scrolled){
            $('html,body').stop().animate({ scrollTop: $('label.radGroup2').offset().top-100 }, 200);
            newitem.scrolled++;
         }
      }
   ...
   this.save = function(){
      if (!this.checkEmpty() && !this.checkNotChecked() && !this.showMinusWords())
         $.post(post_url,{
            a: "item_edit",
            b: this.collectData(),
            ...
            if (result == 1)
               $('#content-area').html("<div id='add_result_msg' style='width:100%;text-align:center;'>Объявление успешно отредактировано. Посмотреть его можно <a href='item.php?num="+(newitem.temp_item_num != -1 ? newitem.temp_item_num : result)+"' target='_blank'>по ссылке</a>.</div>");
   ...
   this.collectData = function(){
      ...
      data.metro = $('#metro_input').attr("metro_code");
